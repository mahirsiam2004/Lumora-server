...
async function run() {
  try {
    ...
    // Create indexes
    await servicesCollection.createIndex({ service_name: 'text', description: 'text' });
    ...
    app.patch('/api/decorators/:email', verifyToken, verifyDecorator, async (req, res) => {
      const email = req.params.email;
    ...
    app.get('/api/bookings/decorator/:email', verifyToken, verifyDecorator, async (req, res) => {
      const email = req.params.email;
    ...
    app.get('/api/bookings', verifyToken, verifyAdmin, async (req, res) => {
      ...
      const bookings = await bookingsCollection.find(query).sort(sortOptions).toArray();
      res.send(bookings);
    });
    ...
    app.patch('/api/bookings/:id/assign', verifyToken, verifyAdmin, async (req, res) => {
      const id = req.params.id;
    ...
    app.patch('/api/bookings/:id/status', verifyToken, verifyDecorator, async (req, res) => {
      const id = req.params.id;
    ...
    app.delete('/api/bookings/:id', verifyToken, async (req, res) => {
      ...
      if (booking.isPaid) {
        return res.status(400).send({ message: 'Cannot cancel paid booking. Please contact support.' });
      }
    ...
    app.get('/api/payments/decorator/:email', verifyToken, verifyDecorator, async (req, res) => {
      const email = req.params.email;
    ...
    app.get('/api/analytics/dashboard', verifyToken, verifyAdmin, async (req, res) => {
      ...
      const paidBookings = await bookingsCollection.countDocuments({ isPaid: true });
